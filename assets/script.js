document.addEventListener('DOMContentLoaded',()=>{
  ...
  @media(max-width:1040px){
    ...
    .hero-grid,.pagehero-visual,.split,.photo-grid,.editorial-grid,.cta-panel{grid-template-columns:1fr!important}.cards-3,.grid3,.decision-table{grid-template-columns:repeat(2,1fr)!important}.stats-grid,.trust-grid,.credential-rail .container{grid-template-columns:repeat(2,1fr)!important}.footer-grid{grid-template-columns:1fr 1fr!important}
  }
  @media(max-width:680px){
    ...
    .hero-home,.pagehero{padding:46px 0 52px}.hero-grid,.pagehero-visual,.split{gap:30px}.hero h1,.pagehero h1{font-size:clamp(2.45rem,12vw,3.6rem);line-height:1.01}.hero-lead,.lead,.dark-lead{font-size:.98rem;line-height:1.7}
    .section,main>section:not(.hero):not(.pagehero):not(.trust-strip):not(.credential-rail):not(.stats-band):not(.darksec):not(.alt),.section-dark,.darksec,.alt{padding:54px 0}.stats-band{padding:46px 0}.section-heading{margin-bottom:28px}.cards-3,.grid3,.grid2,.decision-table,.stats-grid,.trust-grid,.credential-rail .container,.proof-band,.footer-grid{grid-template-columns:1fr!important}.credential-rail span{border-right:0;border-bottom:1px solid var(--line);padding:14px 16px}.actions{display:grid;align-items:stretch;gap:10px}.actions .btn{width:100%}.cta-panel,.cta{padding:28px 22px;border-radius:16px}.cta-panel .btn{width:100%}.image-feature,.pagehero-image{min-height:310px}.service-card,.highlight-card,.card{padding:24px}.decision-table>div{padding:24px}.stat-card{padding:22px}.process-card{padding:8px 20px}.process-card>div{padding:17px 0}.employer-lens,.service-difference{padding:28px 22px}.outcome-item{padding:17px 18px}.stats-intro{display:block}.stats-source{margin-top:16px;text-align:left}.footer-logo{width:min(230px,82%)}h2{font-size:2.2rem}.stat-number{font-size:2.35rem}
  }`;
  document.head.appendChild(style);

  const links=document.querySelector('.nav-links, .links');
  if(links){
    links.innerHTML='<a href="organisations.html">Why It Matters</a><a href="employee-support.html">How We Work</a><a href="about.html">About</a><a class="btn btn-gold" href="contact.html">Contact Steven</a>';
  }
  ...
  if(footerEmployer) footerEmployer.innerHTML='<strong>Start Here</strong><a href="organisations.html">Why It Matters</a><a href="employee-support.html">How We Work</a><a href="about.html">About</a><a href="contact.html">Contact Steven</a>';

  const nav=document.querySelector('.nav-wrap, body>header nav');
  if(!nav||!links) return;
  const btn=document.createElement('button');
  btn.className='menu-toggle';btn.type='button';btn.setAttribute('aria-label','Open navigation');btn.setAttribute('aria-expanded','false');btn.innerHTML='<span></span><span></span><span></span>';
